module Users
  class AccountByRoleFilter < Avo::Filters::BooleanFilter
    self.name = self.name = I18n.t(:'filters.by_role_filter.name')
    self.visible = lambda {
      Current.user.account_admin? && !Current.user.admin?
    }
    ...
    def options
      ...
      #   end
      #   query = query.select('roles.name, MIN(roles.id) as id, MIN(roles.account_id) as account_id')
      #     .group('roles.name')
      #     .order(:name)
      # end
