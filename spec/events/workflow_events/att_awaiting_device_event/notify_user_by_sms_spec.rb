RSpec.describe Issues::WorkflowEvents::AttAwaitingDeviceEvent::NotifyUserBySms do
  describe "#call" do
    subject(:call) do
      described_class.call(resource_id: issue.id, resource_class: issue.class.to_s,
                           from:, to: "awaiting_device", triggering_event: "triggering_event",
    ...
    let(:issue) { create(:issue, assignee: Current.user, status: "awaiting_parts") }
    ...
    context "When event issue_order_print is create and notification rule are set " do
      ...
      let!(:notification_rule) do
        create(:customer_notification_rule,
               ...
               trigger_events: %w[issue_awaiting_device],
               channel: 'sms')
      end
      it "will not send an sms" do
        expect(send_sms).not_to receive(:call)
